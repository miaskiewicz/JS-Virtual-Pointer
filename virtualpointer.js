var virtualpointer = function() { 
    ...
    function build_click_event_queue(element, duration, is_mobile) {
        ...
        if (is_mobile) {
            event_queue.push({type: "touchstart", pageX: x_offset, pageY: x_offset, screenX: x_offset, screenY: y_offset, timestamp: last_timestamp, element: element, is_touch_event: touch_event});
            event_queue.push({type: "touchend", pageX: x_offset, pageY: x_offset, screenX: x_offset, screenY: y_offset, timestamp: last_timestamp + default_click_duration, element: element, is_touch_event: touch_event});
        } else {
            event_queue.push({type: "mousedown", pageX: x_offset, pageY: x_offset, screenX: x_offset + default_screen_x_offset, screenY: y_offset + default_screen_y_offset, timestamp: last_timestamp, element: element, is_touch_event: touch_event});
            event_queue.push({type: "mouseup", pageX: x_offset, pageY: x_offset, screenX: x_offset + default_screen_x_offset, screenY: y_offset + default_screen_y_offset, timestamp: last_timestamp + default_click_duration, element: element, is_touch_event: touch_event});
        }
        event_queue.push({type: "click", pageX: x_offset, pageY: x_offset, screenX: x_offset + default_screen_x_offset, screenY: y_offset + default_screen_y_offset, timestamp: last_timestamp + default_click_duration + 10, element: element, is_touch_event: touch_event});
    }
    ...
    function build_flick_event_queue(element, duration) {
        ...
        event_queue.push({type: "touchstart", pageX: mouse_position.x, pageY: mouse_position.y, screenX: mouse_position.x, screenY: mouse_position.y, timestamp: 0, element: element, is_touch_event: true});
        ...
        for (var i = 1; i <= increments; i++) {
            var new_x_pos = Math.round(x_distance / increments * i) + mouse_position.x,
                new_y_pos = Math.round(y_distance / increments * i) + mouse_position.y;
            event_queue.push({type: "touchmove", pageX: new_x_pos, pageY: new_y_pos, screenX: new_x_pos, screenY: new_y_pos, timestamp: i * default_interval, element: element, is_touch_event: true});
        }

        // get timestamp of last event in queue
        var last_timestamp = (event_queue.length) ? event_queue[event_queue.length - 1].timestamp : 0;

        event_queue.push({type: "touchend", pageX: x_offset, pageY: y_offset, screenX: x_offset, screenY: y_offset, timestamp: last_timestamp, element: element, is_touch_event: true});
    }
